...
# Set OpenAI API key from Streamlit secrets BEFORE any other imports
...
class IllustrationTool(Tool):
    ...
    arg: str = "The story text for which to create an illustration"
...
class AudiobookTool(Tool):
    name: str = "audiobook_tool"
    description: str = "Creates an audio narration of the story"
    arg: str = "The story text to narrate"
...
class StoryPublicationFlow(Workflow):
    ...
    async def generate_content(self, ctx: Context, ev: FinalStoryPackage) -> StopEvent:
        ...
        teaser_agent = AgentPro(tools=[TeaserTool()])
        teaser = teaser_agent(f"Create a teaser for this story: {final_story}")
        ...
        illustration_agent = AgentPro(tools=[IllustrationTool()])
        illustration_prompt = illustration_agent(f"Create an illustration for this story: {final_story}")
